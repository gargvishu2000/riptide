...
const Cart = () => {
  ...
  const fetchProductData = async () => {
    const findItem = products.find(temp => temp._id == cartItem);
      ...
      <div>
        {
          cartData.map((item, index) => {
            ...
            return (
              <div key={index} className='py-3 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols[4fr_2fr_0.5fr] items-center gap-4'>
                <div className='flex items-start gap-6'>
                  <img src={productData.image[0]} className='w-16 sm:w-20'/>
                  <div>
                    <p className='text-xs sm:text-lg font-medium'>{productData.name}</p>
                    <div className='flex items-center mt-2 gap-5'>
                      <p className='text-xl'>{currency}{productData.price}</p>
                ...
                <img onClick={()=> updateQuantity(item._id,item.size,0)} src={assets.bin_icon} className='w-4 mr-4 sm:mr-6 cursor-pointer' alt="" />
